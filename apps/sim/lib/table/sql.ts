 ...
 * Cast is chosen by `columnType` when provided (`'date'` → `::timestamp`, `'number'` → `::numeric`),
 * otherwise inferred from the value type (string → `::timestamp`, number → `::numeric`).
 */
function buildComparisonClause(
  ...
  const isDate = columnType === 'date' || (columnType === undefined && typeof value === 'string')
  if (isDate) {
